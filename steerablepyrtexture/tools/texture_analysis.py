...
from ..pyramids import SteerablePyramidFreq
...
def texture_analyze(img,Nsc,Nor,Na, vector_output=False):
    ...
    #pyr = SteerablePyramidFreq(im0,is_complex=True)
    pyr0 = SteerablePyramidFreq(im0, height=Nsc, order=Nor-1, twidth=1, is_complex=True)
    ...
    mpyr = SteerablePyramidFreq(rpyr[numbands-1], height=0, order=0, is_complex=False)
    ...
    for n in range(Nsc,0,-1):
        ...
        fakepyr = SteerablePyramidFreq(np.zeros((Nly,Nlx)), height=1, order=3, twidth=1, is_complex=False)
...
def feature_vector_names(Nsc, Nor, Na):
    '''
    Generate an ordered list of the descriptors of each feature within the 
    feature vector generated by the texture_analyze() function with the same
    input parameters.
    ...
    Na : int
        Size of the neighborhood of cross-correlations (Na x Na).

    Returns
    -------
    feature_names : list
        A list of strings with the same size and ordering as the features in
        the vector output of texture_analyze().
    ...
    feature_names = np.array(['Pixel Stats: Mean',
                              'Pixel Stats: Variance',
                              'Pixel Stats: Skew',
                              'Pixel Stats: Kurtosis',
                              'Pixel Stats: Min',
                              'Pixel Stats: Max'])
    for i in range(Nsc+1):
        feature_names = np.append(feature_names, f'Pixel LP Stats: Skew {i}')
    for i in range(Nsc+1):
        feature_names = np.append(feature_names, f'Pixel LP Stats: Kurtosis {i}')
        
    for i in range(Nsc+1):
        for j in range(Na):
            for k in range(Na):
                feature_names = np.append(feature_names, 
                                         f'Real AutoCorr: {i}:({j-Na2},{k-Na2})')
    ...
    for i in range(Nsc):
        for j in range(Nor):
            for k in range(Na):
                for n in range(Na):
                    feature_names = np.append(feature_names, 
                                             f'Mag AutoCorr: ({i},{j}):({k-Na2},{n-Na2})')
                    
    feature_names = np.append(feature_names, f'Mag Means: HP')
    for i in range(Nsc):
        for j in range(Nor):
            feature_names = np.append(feature_names, f'Mag Means: ({i},{j})')
    
    feature_names = np.append(feature_names, f'Mag Means: LP')
    
    for i in range(Nsc+1):
        for j in range(Nor):
            for k in range(Nor):
                feature_names = np.append(feature_names, 
                                         f'Cousin Mag Corr: {i}:({j},{k})')
    
    for i in range(Nsc):
        for j in range(Nor):
            for k in range(Nor):
                feature_names = np.append(feature_names, 
                                         f'Parent Mag Corr: {i}:({j},{k})')
    
    for i in range(Nsc+1):
        for j in range(2*Nor):
            for k in range(2*Nor):
                feature_names = np.append(feature_names, 
                                         f'Cousin Real Corr: {i}:({j},{k})')
    
    for i in range(Nsc):
        for j in range(2*Nor):
            for k in range(2*Nor):
                feature_names = np.append(feature_names, 
                                         f'Parent Real Corr: {i}:({j},{k})')
    
    feature_names = np.append(feature_names, 'HP Variance')
